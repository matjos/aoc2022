...
public class DEC_7_B_Tests : BaseTest
{
    ...
    private static float Sum(string[] lines)
    {
        ...
        foreach (var val in dictionarys.Values)
        {
            ...
            if (fileSum < neededFreeSpace -(fileSystem - totalUsedSpace))
            {
                Console.WriteLine($"{val.FolderName} : {fileSum} TO SMALL");
                continue;
            }
            Console.WriteLine($"{val.FolderName} : {fileSum} ");

            results.Add((int)fileSum);
        }
